...
for mb in range(num_mbs):
    ...
    #NEW!!! WE FILTER OUT THE WRONG ONE
     # CHANGED: sample questions + golds for this EI round
    q_batch = sft_q[base:end]
    a_gold_batch = sft_a[base:end]
    gold_norm = [extract_gsm8k_gold(a) for a in a_gold_batch]

    # CHANGED: π_old ← π (sync HF → vLLM) and generate with current policy
    load_policy_into_vllm_instance(model, llm)
    prompts_batch = r1_prompts_from_train(q_batch)   # your helper now accepts a list
    outs = llm.generate(prompts_batch, sampling)

    # CHANGED: score and filter to build D_sft = {(q, o_correct)}
    qs_ok, os_ok = [], []
    for j, o in enumerate(outs):
        text = o.outputs[0].text
        scores = r1_zero_reward_fn(text, gold_norm[j])
        if scores.get("format_reward", 0.0) > 0 and scores.get("answer_reward", 0.0) > 0:
            qs_ok.append(q_batch[j])
            os_ok.append(text)

    # CHANGED: if nothing correct this round, skip update
    if not qs_ok:
        print("!!!!!!!!!!!PASS!!!!!!!!!!!")
        continue

    # CHANGED: tokenize filtered (q, o_correct) pairs for SFT
    tok = run_tokenize_prompt_and_output(qs_ok, os_ok, tokenizer)
    ids  = tok["input_ids"]
    lbls = tok["labels"]
    msk  = tok["response_mask"]  
